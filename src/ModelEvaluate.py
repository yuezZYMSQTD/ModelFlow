...
def plot_ks_cdf(y_true,y_score,pos_label=1,label_map=None,decimals=0,xlabel='Score',ylabel='CumSum',fontsize=12,figsize=(18,8),close=True):
    '''
    ...
    pos_label: int或字符串，positive所属类别。
    label_map: 字典，类别标签含义，默认为{0:'Good',1:'Bad'}。
    decimals: 分割点小数点位数。
    xlabel: 字符串，图片xlabel。
    ylabel: 字符串，图片ylabel。
    fontsize: int，字体大小。
    ...
    crossdens.plot(kind='line', ax=ax)
...
def plot_roc_auc(y_true,y_score,pos_label=1,figsize=(18,8),fontsize=12,title='ROC Curve',close=True):
    '''
    功能: 计算AUC值，并输出ROC曲线。
    y_true: 一维数组或series，代表真实的标签（{0,1}或{-1,1}）。
...
def plot_score_goodbad(y_true,y_score,label_map=None,figsize=(18,8),title=None,
    ...
    if label_map is None:
        label_map={0:'Good',1:'Bad'}
    if len(label_map)!=2:
        raise Exception('label_map只能有两个元素！')
    result={}
    ...
    fig=plt.figure(figsize=figsize)
    ax=fig.add_subplot(111)
    ax.hist(result[keys[0]],bins=100,alpha=0.6,color='b',label=label_map[keys[0]])
    ax.hist(result[keys[1]], bins=100, alpha=0.6, color='r', label=label_map[keys[1]])
    ax.set_xlabel(xlabel,fontsize=fontsize)
...
def test():
    ...
    fig1=plot_score_badratio(y_true, y_score, pos_label=1, bins=10, figsize=(18, 8), title=None, xlabel='Score',
                             ylabel='Frequency', ylabel_secondary='Ratio of Positive', fontsize=12, close=False)
    fig2=plot_score_goodbad(y_true, y_score, label_map={0:'positive',1:'negative'}, figsize=(18, 8), title=None,
                            xlabel='Score', ylabel='Frequency', fontsize=12, close=True)
