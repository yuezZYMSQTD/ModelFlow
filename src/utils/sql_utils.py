...
class MysqlHelper(object):
    ...
    def __init__(self,ip,user,password,db,port=3306,charset='utf8mb4',driver='mysql+pymysql'):
        '''
        数据库连接初始化。
    ...
    def read_table(self,table_name,cols=None):
        '''
        读取数据库表格数据。
        :param table_name: 数据库对应表格名称。
...
def update_sql(data,sql_model,table_name,cols_fixed,cols_set=None):
    '''
    ...
    sql_model.update_noexisted(t1=table_name,t2=table_tmp,set_unique=False,cols_unique=cols_fixed)
    sql_model.execute('drop table if exists %s;'%table_tmp)
