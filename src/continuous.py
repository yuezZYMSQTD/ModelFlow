...
class WoeContinuous(BaseContinuous):
    ...
    def plot_iv(self,top=5,rot=0,figsize=(18,8),close=True):
        '''
        计算变量的IV值并排序，画出直方图。
        top：画图筛选排名前top，若为负值，则不筛选。
        rot：文字旋转角度。
        close：是否关闭图片。
        ...
        ax=fig.add_subplot(111)
        if top<0:
            IVs.plot(kind='bar',rot=rot,legend=False,ax=ax)
        else:
            IVs.head(top).plot(kind='bar',rot=rot,legend=False,ax=ax)
